...
// Append the user `extra-css` snippet slot to the end of the stylesheet. The
// token lives here (not in the source HTML's <style>) so slot management is
// colocated with the HTML slot tokens below and the source HTML stays clean.
const cssWithSlot = `${css}\n\n/* cpn extra-css slot */\n{{EXTRA_CSS}}\n`;
...
export const SITE_STYLES_CSS = ${JSON.stringify(cssWithSlot)};
